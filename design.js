// Avoid Bootstrap Theter dependency error.
...
// Check if we are displaying contents in Safari.
function isSafari() {
...
$(function() {
  ...
  const lastCover = $('#home .cover:last-child');
  home.hide();
  homeMobile.hide();
  if (route === '' || route === 'Home') {
    project.hide();
    if (!isMobile()) {
      ...
      home.imagesLoaded( { background: '.cover' }, function() {
        ...
        home.show();
        if (isSafari()) {
          const lastCoverPrevHeight = lastCover.outerHeight(true);
          const lastCoverNewHeight = lastCoverPrevHeight + footerHeight;
          lastCover.height(lastCoverNewHeight);
        }
        cover.each(function() {
          const anchor = $(this).find('a');
          const h2 = $(this).find('h2');
          const h2Opacity = h2.css('opacity');
          if (h2Opacity === '0') {
            h2.css('display', 'none');
          } else if (h2Opacity < 0.5) {
            anchor.css('cursor', 'default');
            h2.css('display', 'block');
            h2.css('cursor', 'default');
            h2.css('z-index', '-1');
          } else {
            h2.css('display', 'block');
  ...
  $('a[data-url-id]').click(function(event) {
    ...
    const h2Opacity = $(this).find('h2').css('opacity');
    const projectUrl = Cargo.Helper.GetPurlFromPid(id);
    if (h2Opacity !== undefined && h2Opacity < 0.5) {
      return false;
    } else {
      if (projectUrl !== 0) {
